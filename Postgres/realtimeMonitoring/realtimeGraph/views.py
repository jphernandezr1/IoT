...
def get_hot_days_daterange(request):
    data_result = {}

    try:
        measureParam = datetime.fromtimestamp(
            float(request.GET.get("meassure", None)) / 1000
        )
    except:
    ...
    for location in locations:
        stations = Station.objects.filter(location=location)
        locationData = Data.objects.filter(
            station_in=stations, measurementname=selectedMeasure.name,  timegte=start.date(), time_lte=end.date())
        if locationData.count() <= 0:
            continue
    ...
    data_result["end"] = endFormatted
    data_result["data"] = data
    data_result["dates"] = list(set(dates))
