...
void loop() {
  ...
  // Read the analog value from the photoresistor pin
  int photoValue = analogRead(photoresistorPin);

  // Convert the analog value to a light intensity value (adjust as needed)
  // This conversion might require some experimentation based on your photoresistor's characteristics
  int lightIntensity = map(photoValue, 0, 1023, 0, 100);  // Map to a 0-100 range

  // Create a JSON message for the light intensity value
  String jsonLight = "{\"value\": " + String(lightIntensity) + "}";
  char payloadLight[jsonLight.length() + 1];
  jsonLight.toCharArray(payloadLight, jsonLight.length() + 1);

  // Publish the light intensity value to an MQTT topic
  client.publish("luminosidad/bogota/" HOSTNAME, payloadLight, false);
